...
class idrid_dset(BaseDataset):
    def __init__(
        self, data_root, data_list, trs_form, seed=0, n_sup=54, split="val"
    ):
        super(idrid_dset, self).__init__(data_list)
...
def build_idridloader(split, all_cfg, seed=0):
    ...
    n_sup = cfg.get("n_sup", 54)
    # build transform
    trs_form = build_transfrom(cfg)
    dset = idrid_dset(cfg["data_root"], cfg["data_list"], trs_form, seed, n_sup)
...
def build_idrid_semi_loader(split, all_cfg, seed=0):
    cfg_dset = all_cfg["dataset"]
    ...
    workers = cfg.get("workers", 2)
    batch_size = cfg.get("batch_size", 1)
    n_sup = 54 - cfg.get("n_sup", 14)

    # build transform
    trs_form = build_transfrom(cfg)
    trs_form_unsup = build_transfrom(cfg)
    dset = idrid_dset(cfg["data_root"], cfg["data_list"], trs_form, seed, n_sup, split)
